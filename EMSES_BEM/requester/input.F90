...
! /digcon/
           ...
           ijdiag = 0
           iadiag = 0
       odoms(:,:) = reshape((/-9999,-9999,1,-9999,-9999,1,-9999,-9999,1/),(/3,3/))
         isort(1) = 500
          idim_sp = 3
         imdig(1) = 0
        ipadig(1) = 0
...
!
          do iepl=1,inepl
              ...
              ipc_ej(iepl)=0
              radi_ej(iepl)=0.0d0
              cntr_ej(1:3,iepl)=dmiss
              edge_ej(1:2,iepl)=dmiss
              xlej(iepl)=dmiss
              xuej(iepl)=dmiss
              ylej(iepl)=dmiss
              yuej(iepl)=dmiss
              ...
              zmine(iepl)=dmiss
              esfalign(iepl) = 1
              esfradius(iepl) = 0.0d0
              esfedge(1,iepl) = dmiss
              esfedge(2,iepl) = dmiss
              esfcoord(1,iepl) = dmiss
              esfcoord(2,iepl) = dmiss
              esfcoord(3,iepl) = dmiss
...
!              npsnap(is) = npsnap(is) + 1
      ...
      if(odoms(1,1).eq.-9999) then
        odoms(1:3,1:3) = reshape((/0,nx,1,0,ny,1,0,nz,1/),(/3,3/))
      end if
      odoms(1,1) = max(odoms(1,1),  -1)
      odoms(2,1) = min(odoms(2,1),nx+1)
      odoms(3,1) = min(max(odoms(3,1),1),max(odoms(2,1)-odoms(1,1),1))
      odoms(1,2) = max(odoms(1,2),  -1)
      odoms(2,2) = min(odoms(2,2),ny+1)
      odoms(3,2) = min(max(odoms(3,2),1),max(odoms(2,2)-odoms(1,2),1))
      odoms(1,3) = max(odoms(1,3),  -1)
      odoms(2,3) = min(odoms(2,3),nz+1)
      odoms(3,3) = min(max(odoms(3,3),1),max(odoms(2,3)-odoms(1,3),1))
...
!!! treatments for deprecated parameters (20211009)
         do iepl=1,sum(nepl(1:nspec))
           if(abs(ipcpl(iepl)).ge.1.and.abs(ipcpl(iepl)).le.npc) then
             if((nemd(iepl).eq. 1.and.xmaxe(iepl).ne.dmiss).or. &
            &   (nemd(iepl).eq.-1.and.xmine(iepl).ne.dmiss).or. &
            &   (nemd(iepl).eq. 2.and.ymaxe(iepl).ne.dmiss).or. &
            &   (nemd(iepl).eq.-2.and.ymine(iepl).ne.dmiss).or. &
            &   (nemd(iepl).eq. 3.and.zmaxe(iepl).ne.dmiss).or. &
            &   (nemd(iepl).eq.-3.and.zmine(iepl).ne.dmiss)) then
               ipcpl(iepl) = -abs(ipcpl(iepl))
               geom_ej(iepl) = 1
               xlej(iepl) = xmine(iepl)
               xuej(iepl) = xmaxe(iepl)
               ylej(iepl) = ymine(iepl)
               yuej(iepl) = ymaxe(iepl)
               zlej(iepl) = zmine(iepl)
               zuej(iepl) = zmaxe(iepl)
             end if
           end if
!
           if(omni_ej(iepl).eq.-1.and.omniemit(iepl).eq.1) then
             omni_ej(iepl) = 1
           else if(omni_ej(iepl).eq.-1) then
             omni_ej(iepl) = 0
           end if
!
           if(ipc_ej(iepl).eq.0.and.ipcpl(iepl).gt.0) then
             ipc_ej(iepl) = ipcpl(iepl)
           end if
         end do
!
         do iepl=1,sum(nepl(1:nspec))
           if(ipc_ej(iepl).ge.1.and.ipc_ej(iepl).le.npc) then
             ipc = ipc_ej(iepl)
             if(geotype(ipc).eq.0.or.geotype(ipc).eq.1) then
               ...
               geom_ej(iepl) = 2
               algn_ej(iepl) = cylinder(ipc)%align
               radi_ej(iepl) = cylinder(ipc)%radius
               cntr_ej(1:2,iepl) = cylinder(ipc)%axis(1:2)
               edge_ej(1:2,iepl) = cylinder(ipc)%edge(1:2)
             else if(geotype(ipc).eq.3) then
               geom_ej(iepl) = 3
               radi_ej(iepl) = sphere(ipc)%radius
               cntr_ej(1:3,iepl) = sphere(ipc)%center(1:3)
               if(myid.eq.0) print*, "geom,radi,cntr",geom_ej(iepl),radi_ej(iepl),cntr_ej(1:3,iepl)
             end if
           else if(ipc_ej(iepl).ge.-npc.and.ipc_ej(iepl).le.-1) then
             ipc_ej(iepl) = abs(ipc_ej(iepl))
           else
             ipc_ej(iepl) = 0
           end if
         end do
...
!----------------------------------------
       ...
       do is=1,nspec
         if(staendinj(2,is).eq.-1) staendinj(2,is) = nstep
       end do
       do iepl=1,inepl
         if(staendinjs(2,iepl).eq.-1) staendinjs(2,iepl) = nstep
       end do
!----------------------------------------
