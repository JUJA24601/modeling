...
!
  ...
  integer(kind=4) :: cylaln
  real(kind=8) :: x,y,z, x1,y1,z1
  real(kind=8) :: cylrad, cylaxs1,cylaxs2, cyledg1,cyledg2
        ...
        if(geotype(ipc).eq.0) then
          do k=0,nz; do j=0,ny; do i=0,nx
           ...
           &   i.ge.xlpc(ipc).and.i.le.xupc(ipc)) then
              ...
              if(chk_dupli(icap).or.carved(icap)) icap = icap - 1
            end if
              ...
              do j=0,ny
                do i=0,nx
                  if((i-cylaxs1)**2+(j-cylaxs2)**2.le.radsq.and. &
                     i.ge.0.and.i.le.nx.and. &
                     j.ge.0.and.j.le.ny.and. &
                     k.ge.0.and.k.le.nz) then
                    icap = icap + 1
                    bdygrid(1,icap) = rfold(i,nx,1)
                    bdygrid(2,icap) = rfold(j,ny,2)
                    bdygrid(3,icap) = rfold(k,nz,3)
...
!
            do k=ceiling(cyledg1)+1,floor(cyledg2)-1
              do j=0,ny
                do i=0,nx
                  if((i-cylaxs1)**2+(j-cylaxs2)**2.le.radsq .and. &
                 &   (i-cylaxs1)**2+(j-cylaxs2)**2.ge.radsqin.and. &
                 &   i.ge.0.and.i.le.nx.and. &
                 &   j.ge.0.and.j.le.ny.and. &
                 &   k.ge.0.and.k.le.nz) then
                    icap = icap + 1
...
!
                    x = cylaxs1 - bdisp
                    if(abs(x-ceiling(x)).gt.mingap.and. &
                   &   x.ge.0.and.x.le.nx.and. &
                   &   j.ge.0.and.j.le.ny.and. &
                   &   k.ge.0.and.k.le.nz) then
                      icap = icap + 1
                      bdygrid(1,icap) = rfold(x,slx,1)
...
!
                    x = cylaxs1 + bdisp
                    if(abs(x-floor(x)).gt.mingap.and. &
                   &   x.ge.0.and.x.le.nx.and. &
                   &   j.ge.0.and.j.le.ny.and. &
                   &   k.ge.0.and.k.le.nz) then
                      icap = icap + 1
...
!
                    y = cylaxs2 - bdisp
                    if(abs(y-ceiling(y)).gt.mingap.and. &
                   &   i.ge.0.and.i.le.nx.and. &
                   &   y.ge.0.and.y.le.ny.and. &
                   &   k.ge.0.and.k.le.nz) then
                      icap = icap + 1
                      bdygrid(1,icap) = rfold(i,nx,1)
                      bdygrid(2,icap) = rfold(y,sly,2)
...
!
                    y = cylaxs2 + bdisp
                    if(abs(y-floor(y)).gt.mingap.and. &
                   &   i.ge.0.and.i.le.nx.and. &
                   &   y.ge.0.and.y.le.ny.and. &
                   &   k.ge.0.and.k.le.nz) then
                      icap = icap + 1
              ...
              if(cyledg1.ne.ceiling(cyledg1)) then
                do j=0,ny
                  do i=0,nx
                   ...
                   &   i.ge.0.and.i.le.nx.and. &
                   &   j.ge.0.and.j.le.ny.and. &
                   &   cyledg1.ge.0.and.cyledg1.le.nz) then
                      icap = icap + 1
                      bdygrid(1,icap) = rfold(i,nx,1)
                      bdygrid(2,icap) = rfold(j,ny,2)
              ...
              if(cyledg2.ne.floor(cyledg2)) then
                do j=0,ny
                  do i=0,nx
                   ...
                   &   i.ge.0.and.i.le.nx.and. &
                   &   j.ge.0.and.j.le.ny.and. &
                   &   cyledg2.ge.0.and.cyledg2.le.nz) then
                      icap = icap + 1
                      bdygrid(1,icap) = rfold(i,nx,1)
                      bdygrid(2,icap) = rfold(j,ny,2)
...
!          if(medi(1,i+xl,j+yl,k+zl).ne.1.or.medi(1,i+1+xl,j+yl,k+zl).ne.1) then
...
!          if(medi(1,i+xl,j+yl,k+zl).ne.1.or.medi(1,i+xl,j+1+yl,k+zl).ne.1) then
...
!          if(medi(1,i+xl,j+yl,k+zl).ne.1.or.medi(1,i+xl,j+yl,k+1+zl).ne.1) then
...
!          print*, "  ",x,y,z,"mp(BZ) = 0", myid
  ...
  function carved(i3cap)
    implicit none
    logical :: carved
    integer(kind=4),intent(in) :: i3cap
    integer(kind=4) :: icav, i,j,k
    integer(kind=4) :: cylaln
    real(kind=8) :: cylrad, cylaxs1,cylaxs2, cyledg1,cyledg2
    real(kind=8) :: radsq
...
!      cylaxs1 = xholec
!      cylaxs2 = yholec
!      cyledg1 = ubhole - 1.0d0
!      cyledg2 = ubhole + 1.0d0

      !GEOTYPE2
!      if(geotype(icav).eq.2) then
!        radsq = cylrad**2
!        if(cylaln.eq.3) then
!          if((i-cylaxs1)**2+(j-cylaxs2)**2.le.radsq.and. &
!             k.ge.cyledg1.and.k.le.cyledg2) then
!          if(i.gt.cylaxs1-rhole.and.i.lt.cylaxs1+rhole.and. &
!         &   k.ge.cyledg1.and.k.le.cyledg2) then
!             carved = .true.
!            exit
!          end if
