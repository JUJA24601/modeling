...
!       ------------- velocity v
        renv    = dth*dri
!       ------------- charge q
        renq    = qm1*((dth*dri)**2)*dri  ! 内部単位系は単位系*これ
!       ------------- electric field e
        rene    = qm1*(dth**2)*dri
!       ------------- magnetic field b
        renb    = qm1*dth
...
!-------------------- normalization for fields
      ...
      eb(BY,:,:,:,:) = eb(BY,:,:,:,:)*renb
      eb(BZ,:,:,:,:) = eb(BZ,:,:,:,:)*renb
      ebamp(EX,:) = ebamp(EX,:)*rene
      ebamp(EY,:) = ebamp(EY,:)*rene
      ebamp(EZ,:) = ebamp(EZ,:)*rene
      ebamp(BX,:) = ebamp(BX,:)*renb
      ebamp(BY,:) = ebamp(BY,:)*renb
      ebamp(BZ,:) = ebamp(BZ,:)*renb
      aj(:,:,:,:,:) = aj(:,:,:,:,:)*renj
      rho(:,:,:,:,:) = rho(:,:,:,:,:)*renrho
      phi(:,:,:,:,:) = phi(:,:,:,:,:)*renphi
...
!-------------------- normalization for PML coefficients
      pmlc(2,:,1) = pmlc(2,:,1)*renv
      pmlc(4,:,1) = pmlc(4,:,1)*renv
      pmlc(6,:,1) = pmlc(6,:,1)*renv
      pmlc(2,:,2) = pmlc(2,:,2)/renv
      pmlc(4,:,2) = pmlc(4,:,2)/renv
      pmlc(6,:,2) = pmlc(6,:,2)/renv
