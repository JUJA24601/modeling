...
!     --------------- 
      b0   = wc/qm(1)
!
      csz  = cos(pi*phiz /180.0d0)
      snz  = sin(pi*phiz /180.0d0)
      csxy = cos(pi*phixy/180.0d0)
      snxy = sin(pi*phixy/180.0d0)
!
      t11 =  csz * csxy
      t12 =      - snxy
      t13 =  snz * csxy
...
!        print*,"medges:"
!        do sd=0,nnode-1
!          print*,"  sd=",sd,"  ",medges(:,1,sd+1)-sdoms(1,1,sd+1), &
!         &       " ",medges(:,2,sd+1)-sdoms(1,2,sd+1), &
!         &       " ",medges(:,3,sd+1)-sdoms(1,3,sd+1)
!        end do
!      end if


!-------------------- set index for medium coefficients
!!!      call medium


!-------------------- initialization for potential calculation
      if((pftmode.eq.6).or. &
     &   (pftmode.eq.0.and.nodes(1)*nodes(2).eq.1.and.nx*ny.gt.100)) then
        nodes(5) = getgcd(ny,nnode)
        nyslc = ny/nodes(5)
        nodes(4) = getgcd(nnode/nodes(5),nx)
        nxslc = nx/nodes(4)
        snode = nodes(4)*nodes(5)
        nzslc = nz/nnode
        idxbr = mod(myid,nodes(4))
        idybr = mod(int(myid/nodes(4)),nodes(5))
        pftmode = 6
      else if((pftmode.eq.5).or. &
     &        (pftmode.eq.0.and.nodes(2)*nodes(3).eq.1.and.ny*nz.gt.100)) then
        nodes(4) = getgcd(nz,nnode)
        nzslc = nz/nodes(4)
        nodes(5) = getgcd(nnode/nodes(4),ny)
        nyslc = ny/nodes(5)
        snode = nodes(4)*nodes(5)
        nxslc = nx/nnode
        idybr = mod(myid,nodes(4))
        idzbr = mod(int(myid/nodes(4)),nodes(5))
        pftmode = 5
      else if((pftmode.eq.4).or. &
     ...
     &        (pftmode.eq.0.and.nodes(2)*nodes(3).eq.1)) then
      ...
      if(myid.eq.0) print*, "pftmode,snode =", pftmode,snode
!
      if(myid.lt.snode) then
        if(pftmode.eq.1) then
          ...
          sxl  = nxslc*myid;  sxu  = sxl  + nxslc
          stxl = 0;           stxu = stxl + nx
          syl  = nyslc*idybr; syu  = syl  + nyslc
          styl = nyslc*idybr; styu = styl + nyslc
          szl  = nzslc*idzbr; szu  = szl  + nzslc
          stzl = nzslc*idzbr; stzu = stzl + nzslc
        else if(pftmode.eq.6) then
          sxl  = nxslc*idxbr; sxu  = sxl  + nxslc
          stxl = nxslc*idxbr; stxu = stxl + nxslc
          syl  = nyslc*idybr; syu  = syl  + nyslc
          styl = nyslc*idybr; styu = styl + nyslc
          szl  = nzslc*myid;  szu  = szl  + nzslc
          stzl = 0;           stzu = stzl + nz
          ...
          stxl = nxslc*myid; stxu = stxl + nxslc
          syl  = nyslc*myid; syu  = syl  + nyslc
          styl = nyslc*myid; styu = styl + nyslc
          szl  = nzslc*myid; szu  = szl  + nzslc
          stzl = nzslc*myid; stzu = stzl + nzslc
        end if
      else
        sxl  = nx; sxu  = sxl
        stxl = nx; stxu = stxl
...
!
      ...
      if(myid.eq.0) print*, "n{w,d,h}slc =", nwslc,ndslc,nhslc
!
     ...
     &   (pftmode.eq.2.and.nfbnd(2).eq.0)) then
        lslice(1) = mod(myid-1+snode,snode)
        uslice(1) = mod(myid+1+snode,snode)
      else if((pftmode.eq.3.and.nfbnd(1).eq.0).or. &
     &   (pftmode.eq.4.and.nfbnd(3).eq.0).or. &
     &   (pftmode.eq.5.and.nfbnd(1).eq.0).or. &
     &   (pftmode.eq.6.and.nfbnd(3).eq.0)) then
        lslice(1) = mod(myid-1+nnode,nnode)
        uslice(1) = mod(myid+1+nnode,nnode)
      else
        lslice(1) = myid - 1
        uslice(1) = myid + 1
     ...
     &   (pftmode.eq.2.and.mtd_vbnd(2).eq.0)) then
        lslice(2) = mod(myid-1+snode,snode)
        uslice(2) = mod(myid+1+snode,snode)
      else if((pftmode.eq.3.and.mtd_vbnd(1).eq.0).or. &
     &   (pftmode.eq.4.and.mtd_vbnd(3).eq.0).or. &
     &   (pftmode.eq.5.and.mtd_vbnd(3).eq.0).or. &
     &   (pftmode.eq.6.and.mtd_vbnd(3).eq.0)) then
        lslice(2) = mod(myid-1+nnode,nnode)
        uslice(2) = mod(myid+1+nnode,nnode)
      else
        lslice(2) = myid - 1
        uslice(2) = myid + 1
...
!
      if(pftmode.eq.1) then
        ...
        allocate(xtype(1),ytype(0:nodes(4)-1),ztype(0:nodes(5)-1))
        xtype(1) = 0
        ytype(0:nodes(4)-2) = 0; ytype(nodes(4)-1) = 1
        ztype(0:nodes(5)-2) = 0; ztype(nodes(5)-1) = 1
        allocate(istatus(MSS,NCOMREQ),ireqs(NCOMREQ))
      else if(pftmode.eq.6) then
        allocate(xtype(0:nodes(4)-1),ytype(0:nodes(5)-1),ztype(1))
        xtype(0:nodes(4)-2) = 0; xtype(nodes(4)-1) = 1
        ytype(0:nodes(5)-2) = 0; ytype(nodes(5)-1) = 1
        ztype(1) = 0
        allocate(istatus(MSS,NCOMREQ),ireqs(NCOMREQ))
      else
...
!-------------------- 
      ...
      call inifft
!
!      allocate(sinfo(8,0:nnode-1))
!      allocate(cinfo(3,0:nodes(3)-1))
!
!      sinfo(1,0:snode-1) = nzslice
!      sinfo(1,snode:nnode-1) = 0
!      cinfo(1,:) = 0
!      layer = 0
!      do slid=0,nnode-1
!        sinfo(2,slid) = layer
!        sinfo(3,slid) = layer + sinfo(1,slid) - 1
!        if(int(sinfo(2,slid)/nzsd).eq.int(sinfo(3,slid)/nzsd).or. &
!       &   slid.ge.snode) then
!          sinfo(4,slid) = sinfo(2,slid)/nzsd
!        else if(slid.lt.snode) then
!          if(myid.eq.0) &
!         &  print*, "Error:", &
!         &  "any slice cannot stride across cuboid boundary in z direction"
!          if(myid.eq.0) &
!         &  print*, "slid,sinfo(2:3,slid)",slid,sinfo(2:3,slid)
!          stop
!        end if
!        if(sinfo(4,slid).lt.nodes(3)) then
!          cinfo(1,sinfo(4,slid)) = cinfo(1,sinfo(4,slid)) + 1
!        end if
!        sinfo(6,slid) = mod(sinfo(2,slid),nzsd)
!        sinfo(7,slid) = mod(sinfo(3,slid),nzsd)
!        if(sinfo(4,slid).lt.nodes(3)) then
!          if(sinfo(6,slid).eq.0) cinfo(2,sinfo(4,slid)) = slid
!          if(sinfo(7,slid).eq.nzsd-1) cinfo(3,sinfo(4,slid)) = slid
!          sinfo(5,slid) = slid - cinfo(2,sinfo(4,slid))
!        else
!          sinfo(5,slid) = -1
!        end if
!        layer = layer + sinfo(1,slid)
!      end do
!      sinfo(8,0:snode-1) = lyslice
!      sinfo(8,snode:nnode-1) = 0
!      if(myid.eq.0) then
!        print*, "sinfo"
!        do slid=0,nnode-1
!          print*, sinfo(:,slid)
...
!
          do ipcg=npcg,0,-1
            pcgs(ipcg+1) = sum(pcgs(1:ipcg))
            ccgs(ipcg+1) = sum(ccgs(1:ipcg))
          end do
        end if
!
        nfcg = 0
        do ipcg=1,npcg
          if(mtd_vchg(ipcg).ne.2) nfcg = nfcg + 1 
        end do
        if(myid.eq.0) print*,"pcgs(1:npcg+1) =", pcgs(1:npcg+1)
        if(myid.eq.0) print*,"ccgs(1:npcg+1) =", ccgs(1:npcg+1)
        if(myid.eq.0) print*,"nfcg =", nfcg
      end if
...
!-------------------- memory allocation for non-blocking comm.
      ncomm = max(ixw*10, nodes(1)*nodes(2)*4 + nnode/nodes(3) + 4, &
...
!-------------------- creating output Files
      if(intfoc.ne.0.and.myid.eq.0) then
        open(FILEEN,file='energy',status='replace')
        close(FILEEN)
        open(FILEPP,file='pprobe',status='replace')
        close(FILEPP)
        open(FILEPB,file='pbody',status='replace')
        close(FILEPB)
        open(FILEIC,file='icur',status='replace')
        close(FILEIC)
        open(FILEOC,file='ocur',status='replace')
        close(FILEOC)
...
! get divisor that is equal to or less than y
