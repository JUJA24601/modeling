...
!
        do k=0,1
        do j=0,1
          call MPI_Type_Create_Hvector((nyslc+j), ncomp(m), &
         &                             int(lwslc*ebyte,MAK), &
         &                             MPI_REAL8, mptype_br(j,k,m,1), mpierr)
          call MPI_Type_Create_Hvector((nzslc+k), 1, &
         &                             int(lwslc*ldslc*ebyte,MAK), &
         &                             mptype_br(j,k,m,1), mptype_br(j,k,m,1), mpierr)
          call MPI_Type_Create_Struct(3, (/1,1,1/), &
         &                            int((/0,0,1/)*ebyte,MAK), &
         &                            (/MPI_LB,mptype_br(j,k,m,1),MPI_UB/), &
         &                            mptype_br(j,k,m,1), mpierr)
          call MPI_Type_Commit(mptype_br(j,k,m,1), mpierr)
        end do
        end do
!
        do k=0,1
        do j=0,1
          call MPI_Type_Create_Hvector((nyslc+j), ncomp(m), &
         &                             int(lxsd(FDB)*ebyte,MAK), &
         &                             MPI_REAL8, mptype_fl(j,k,m,1), mpierr)
          call MPI_Type_Create_Hvector((nzslc+k), 1, &
         &                             int(lxsd(FDB)*lysd(FDB)*ebyte,MAK), &
         &                             mptype_fl(j,k,m,1), mptype_fl(j,k,m,1), mpierr)
          call MPI_Type_Create_Struct(3, (/1,1,1/), &
         &                            int((/0,0,1/)*ebyte,MAK), &
         &                            (/MPI_LB,mptype_fl(j,k,m,1),MPI_UB/), &
         &                            mptype_fl(j,k,m,1), mpierr)
          call MPI_Type_Commit(mptype_fl(j,k,m,1), mpierr)
        end do
        end do
      end do
...
!
        do j=0,1
        do i=0,1
          call MPI_Type_Create_Hvector((nyslc+j), (nxslc+i)*ncomp(m), &
         &                             int(lwslc*ebyte,MAK), &
         &                             MPI_REAL8, mptype_br(i,j,m,2), mpierr)
          call MPI_Type_Create_Struct(3, (/1,1,1/), &
         &                            int((/0,0,lwslc*ldslc/)*ebyte,MAK), &
         &                            (/MPI_LB,mptype_br(i,j,m,2),MPI_UB/), &
         &                            mptype_br(i,j,m,2), mpierr)
          call MPI_Type_Commit(mptype_br(i,j,m,2), mpierr)
        end do
        end do
!
        do j=0,1
        do i=0,1
          call MPI_Type_Create_Hvector((nyslc+j), (nxslc+i)*ncomp(m), &
         &                             int(lxsd(FDB)*ebyte,MAK), &
         &                             MPI_REAL8, mptype_fl(i,j,m,2), mpierr)
          call MPI_Type_Create_Struct(3, (/1,1,1/), &
         &                            int((/0,0,lxsd(FDB)*lysd(FDB)/)*ebyte,MAK), &
         &                            (/MPI_LB,mptype_fl(i,j,m,2),MPI_UB/), &
         &                            mptype_fl(i,j,m,2), mpierr)
          call MPI_Type_Commit(mptype_fl(i,j,m,2), mpierr)
        end do
        end do
      end do
...
!-------------------- 
      do m=1,2
        ebyte = 8*ncomp(m)
!
        call MPI_Type_Contiguous(lxsd(FDB)*lysd(FDB)*ncomp(m), &
       &                         MPI_REAL8, mptype_xy(m,1), mpierr)
        call MPI_Type_Commit(mptype_xy(m,1), mpierr)
!
        call MPI_Type_Contiguous(lwslc*ldslc*ncomp(m), &
       &                         MPI_REAL8, mptype_xy(m,2), mpierr)
        call MPI_Type_Commit(mptype_xy(m,2), mpierr)
      end do
...
!-------------------- 
      do m=1,2
        ebyte = 8*ncomp(m)
!
        call MPI_Type_Vector(lysd(FDB), ncomp(m), lxsd(FDB)*ncomp(m), &
       &                     MPI_REAL8, mptype_yz(m,1), mpierr)
        call MPI_Type_Create_Hvector(lzsd(FDB), 1, &
       &                             int(lxsd(FDB)*lysd(FDB)*ebyte,MAK), &
       &                             mptype_yz(m,1), mptype_yz(m,1), mpierr)
        call MPI_Type_Create_Struct(2, (/1,1/), int((/0,1/)*ebyte,MAK), &
       &                            (/mptype_yz(m,1),MPI_UB/), &
       &                            mptype_yz(m,1), mpierr)
        call MPI_Type_Commit(mptype_yz(m,1), mpierr)
!
        call MPI_Type_Vector(ldslc, ncomp(m), lwslc*ncomp(m), &
       &                     MPI_REAL8, mptype_yz(m,2), mpierr)
        call MPI_Type_Create_Hvector(lhslc, 1, int(lwslc*ldslc*ebyte,MAK), &
       &                             mptype_yz(m,2), mptype_yz(m,2), mpierr)
        call MPI_Type_Create_Struct(2, (/1,1/), int((/0,1/)*ebyte,MAK), &
       &                            (/mptype_yz(m,2),MPI_UB/), &
       &                            mptype_yz(m,2), mpierr)
        call MPI_Type_Commit(mptype_yz(m,2), mpierr)
      end do
...
!-------------------- 
      do m=1,2
        ebyte = 8*ncomp(m)
!
        call MPI_Type_Vector(lzsd(FDB), lxsd(FDB)*ncomp(m), &
       &                     lxsd(FDB)*lysd(FDB)*ncomp(m), &
       &                     MPI_REAL8, mptype_xz(m,1), mpierr)
        call MPI_Type_Create_Struct(2, (/1,1/), &
       &                            int((/0,lxsd(FDB)/)*ebyte,MAK), &
       &                            (/mptype_xz(m,1),MPI_UB/), &
       &                            mptype_xz(m,1), mpierr)
        call MPI_Type_Commit(mptype_xz(m,1), mpierr)
!
        call MPI_Type_Vector(lhslc, lwslc*ncomp(m), lwslc*ldslc*ncomp(m), &
       &                     MPI_REAL8, mptype_xz(m,2), mpierr)
        call MPI_Type_Create_Struct(2, (/1,1/), int((/0,lwslc/)*ebyte,MAK), &
       &                            (/mptype_xz(m,2),MPI_UB/), &
       &                            mptype_xz(m,2), mpierr)
        call MPI_Type_Commit(mptype_xz(m,2), mpierr)
      end do
...
!-------------------- MPI datatype & operation definitions for counter
     ...
     &  (/ ispec*4, inpc*ispec, inpc*ispec, &
     &     (inpc+1)*inpc*ispec, (ispec+1)/2*2, 2*inpc /), &
     &  int((/ 0, ispec*4*8, ispec*4*8 + inpc*ispec*4, &
     &         ispec*4*8 + 2*inpc*ispec*4, &
     &         ispec*4*8 + (inpc + 3)*inpc*ispec*4, &
     &         ispec*4*8 + ((inpc + 3)*inpc*ispec + (ispec+1)/2*2)*4 /), &
     &      MAK), &
