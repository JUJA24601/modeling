...
!
  ...
  integer(kind=4) :: access, status
  integer(kind=4) :: stats0,stats1
  integer(kind=8) :: dims(2)
  character(len=40) :: filename,dsname
...
!
        ...
        cpmx(:,:) = 0.0d0
!
        status = access('./SNAPSHOT0/cpcmtx.h5',' ')
        if(status.ne.0) then
  JCAPL:  do jcap=1,ncpmx
            if(myid.eq.0) print '(A,i6,3f12.3)','jcap, i, j, k:', &
           &              jcap, bdygrid(1,jcap), bdygrid(2,jcap), bdygrid(3,jcap)
            if(pftmode.eq.1.or.pftmode.eq.2) then
              poi(:,:,:,:,:) = 0.0d0
!
...
!             ------- distribute charge with weight
             ...
             &   k .ge.0.and.k .le.sku) then
              ...
              call poisson(0,rho(1,:,:,:,1:2))
            else
              rho(:,:,:,:,:) = 0.0d0
              poi(:,:,:,:,:) = 0.0d0
!
              xlocal = bdygrid(1,jcap) - xl
              ylocal = bdygrid(2,jcap) - yl
              zlocal = bdygrid(3,jcap) - zl
!
              i = floor(xlocal)
              j = floor(ylocal)
              k = floor(zlocal)
!
              ...
              k1 = k + 1
!
              if(i1.ge.0.and.i.le.iu.and. &
             &   j1.ge.0.and.j.le.ju.and. &
             &   k1.ge.0.and.k.le.ku) then
                x1 = xlocal - i
                y1 = ylocal - j
                z1 = zlocal - k
!               ----- temporary value
...
!               ----- 
                rho(:,:,:,:,:) = 0.0d0
                if(i .ge.0.and.i.le.iu.and. &
               &   j .ge.0.and.j.le.ju.and. &
               &   k1.ge.0.and.k.le.ku) then
                  rho(1,i,j,k1,1) = 1.0d0
                end if
                call poisson(1,rho(1,:,:,:,1:2))
                if(i .ge.0.and.i .le.siu.and. &
               &   j .ge.0.and.j .le.sju.and. &
...
!
                rho(:,:,:,:,:) = 0.0d0
                if(i1.ge.0.and.i1.le.iu.and. &
               &   j .ge.0.and.j .le.ju.and. &
               &   k1.ge.0.and.k1.le.ku) then
                  rho(1,i1,j,k1,1) = 1.0d0
                end if
                call poisson(1,rho(1,:,:,:,1:2))
                if(i1.ge.0.and.i1.le.siu.and. &
               &   j .ge.0.and.j .le.sju.and. &
...
!
                rho(:,:,:,:,:) = 0.0d0
                if(i1.ge.0.and.i1.le.iu.and. &
               &   j1.ge.0.and.j1.le.ju.and. &
               &   k1.ge.0.and.k1.le.ku) then
                  rho(1,i1,j1,k1,1) = 1.0d0
                end if
                call poisson(1,rho(1,:,:,:,1:2))
                if(i1.ge.0.and.i1.le.siu.and. &
               &   j1.ge.0.and.j1.le.sju.and. &
...
!
          ...
          if(myid.eq.0) print*, 'Loading cpcmtx.h5...'
          write(filename,'(a)') './SNAPSHOT0/cpcmtx.h5'
          call hdfopen(filename,fileid,DFACC_READ)
!
          dsname = 'cpmx'
          dims(1) = ncpmx; dims(2) = ncpmx
          call read2d(fileid,dsname,dims,cpmx(1:ncpmx,1:ncpmx),stats0,stats1)
!
...
!
        if(myid.eq.0) then
          ...
          do jcap=1,ncpmx
            ...
            if(ncpmx.gt.0) then
              call read1d(fileid,dsname,dims(1),diff(1:ncpmx),stats0,stats1)
            else
              diff(1:ncpmx) = 0.0d0
            end if
