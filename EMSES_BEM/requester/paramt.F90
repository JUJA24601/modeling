...
!
    ...
    integer(kind=4) :: ixw,inpc,ingap,isj,injs,inepl,inwave,intch,inpprb
    ...
    integer,parameter :: FNR=9,FEB=1,FDB=2,FAJ=3,FRH=4,FRB=5,FPH=6,FJD=7,FRD=8,FSC=9
    integer,parameter :: BNR=2
    integer,parameter :: CNR=6,CEB=1,CAJ=2,CRH=3,CRB=4,CJD=5,CRD=6
    ...
    integer,parameter :: EXY=1,EXZ=2,EYZ=3,EYX=4,EZX=5,EZY=6
    integer,parameter :: BXY=7,BXZ=8,BYZ=9,BYX=10,BZX=11,BZY=12
    integer,parameter :: IEBD=-1,INCB=-2,IPCB=-3
    integer,parameter :: OEBD=-4,ONCB=-5,OPCB=-6,REBD=-14,RNCB=-15,RPCB=-16

    integer,parameter :: FILEEN=101,FILEPP=102,FILEPB=103
    integer,parameter :: FILEIC=104,FILEOC=105

    parameter( ix     = 4 )
    parameter( iy     = 4 )
    parameter( iz     = 4 )
    parameter( ixy    = 4 )
    parameter( inr    = 2097152*2 )
    parameter( ispec  = 6 )
    parameter( ivx    = 32 )
    parameter( ivy    = 32 )
    parameter( ivz    = 32 )
...
!    parameter( lvec   = 1 )
    ...
    type counter
      sequence
      integer(kind=8) :: globalp(ispec,4)
